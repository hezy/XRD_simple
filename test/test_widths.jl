...
@testset "Scherrer size broadening" begin
    ...
    @test Lorentzian_peaks_width([θ], K, 0.0, λ, D_nm)[1] ≈ expected rtol=1e-12
    @test Lorentzian_peaks_width([θ], K, 0.0, λ, D_nm)[1] ≈ 2.953e-3 rtol=1e-3
end

@testset "Stokes-Wilson strain broadening" begin
    # Size term negligible (huge D): FWHM (radians of 2θ) = 4ε tan θ
    θ = deg2rad(30.0)
    @test Lorentzian_peaks_width([θ], 0.9, 0.002, 1.5418, 1e12)[1] ≈ 4 * 0.002 * tan(θ) rtol=1e-6
end

@testset "Caglioti gives FWHM" begin
    θ = deg2rad(25.0)
    U, V, W = 1e-4, -2e-5, 1e-5
    @test Gaussian_peaks_width([θ], U, V, W)[1] ≈ sqrt(U * tan(θ)^2 + V * tan(θ) + W)
end
