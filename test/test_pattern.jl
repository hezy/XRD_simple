...
@testset "sum_peaks" begin
    ...
    result = sum_peaks(θ, θ_list, w_L, w_G)
    @test length(result) == length(θ)
    @test all(result .>= 0)
    @test sum(result) > 0

    single = sum_peaks(θ, [θ_list[1]], w_L, w_G)
...
@testset "compute_xrd_pattern" begin
    ...
    w_L, w_G = compute_peak_widths(θ, peak_width, instrument)
    y = compute_xrd_pattern(θ, indices, λ, a, w_L, w_G)
    @test length(y) == length(θ)
    @test all(y .>= 0)
    @test sum(y) > 0

    Random.seed!(42)
    y1 = compute_xrd_pattern(θ, indices, λ, a, w_L, w_G; noise_level=0.1)
    Random.seed!(43)
    y2 = compute_xrd_pattern(θ, indices, λ, a, w_L, w_G; noise_level=0.1)
    @test y1 != y2

    Random.seed!(42)
    y3 = compute_xrd_pattern(θ, indices, λ, a, w_L, w_G; noise_level=0.1)
    @test y1 == y3
end
